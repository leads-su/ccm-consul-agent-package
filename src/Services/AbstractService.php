...
namespace ConsulConfigManager\Consul\Agent\Services;
 ...
 * @package ConsulConfigManager\Consul\Agent\Services
 */
abstract class AbstractService
{
    ...
    private function initializeClient(): void
    {
        ...
        $this->client = Http::baseUrl(sprintf(
            ...
            'X-Consul-Token'    =>  Arr::get($activeConnection, 'access_token') ?? config('consul.agent.access_token', ''),
        ])->timeout(config('consul.agent.timeout', 15));
    }
    ...
    {
        $connectionName = config('consul.agent.default', 'default');
        $connectionsList = config('consul.agent.connections', [
            'default'       =>  [
                'scheme'    =>  'http',
                'host'      =>  '127.0.0.1',
                'port'      =>  8500,
        ...
        $withAutoSelect = config('consul.agent.auto_select', false);
        $withRandomServer = config('consul.agent.use_random', false);
